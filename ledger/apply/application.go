...
func applyEvalDelta(ac *transactions.ApplicationCallTxnFields, evalDelta basics.EvalDelta, params basics.AppParams, creator, sender basics.Address, balances transactions.Balances, appIdx basics.AppIndex, errIfNotApplied bool) error {
	/*
...
func applyClearState(ac *transactions.ApplicationCallTxnFields, balances transactions.Balances, sender basics.Address, appIdx basics.AppIndex, ad *transactions.ApplyData, steva transactions.StateEvaluator) error {
	...
	if exists {
		...
		if err == nil && pass {
			...
			err = applyEvalDelta(ac, evalDelta, params, creator, sender,
...
func applyOptIn(balances transactions.Balances, sender basics.Address, appIdx basics.AppIndex, params basics.AppParams) error {
	record, err := balances.Get(sender, false)
	if err != nil {
		return err
	}
...
func ApplicationCall(ac *transactions.ApplicationCallTxnFields, header transactions.Header, balances transactions.Balances, spec transactions.SpecialAddresses, ad *transactions.ApplyData, txnCounter uint64, steva transactions.StateEvaluator) (err error) {
	defer func() {
	...
	if ad == nil {
		err = fmt.Errorf("cannot use empty transactions.ApplyData")
		return
	}
	...
	// wrapper around transactions.Balances.
	//
	...
	err = applyEvalDelta(ac, evalDelta, params, creator, header.Sender,
		balances, appIdx, failIfNotApplied)
	if err != nil {
		return err
	}
