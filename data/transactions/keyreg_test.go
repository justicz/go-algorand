...
func TestKeyregApply(t *testing.T) {
	...
	err := tx.KeyregTxnFields.Apply(tx.Header, mockBalances{protocol.ConsensusCurrentVersion}, SpecialAddresses{FeeSink: feeSink}, nil)
	...
	err = tx.KeyregTxnFields.Apply(tx.Header, mockBal, SpecialAddresses{FeeSink: feeSink}, nil)
	require.NoError(t, err)
